...
class AudioRecorderThread(QThread):
    ...
    def process_audio(self, data):
        ...
        try:
            self.message_queue.put(f"Reconociendo y traduciendo voz de {self.input_language} a {self.output_language} con Whisper...")
            
            # Ensure data is in the correct format for Whisper
            audio_data = data.flatten().astype(np.float32)
            
            # Use Whisper for speech recognition and translation
            result = self.whisper_model.transcribe(
                audio_data, 
                language=self.input_language, 
                task="translate" if self.output_language != self.input_language else "transcribe"
            )
            ...
            if texto_original.strip():
                if self.output_language != self.input_language:
                    ...
                    self.message_queue.put(f"Texto traducido (inglés): {texto_original}")  # Whisper translates to English by default
                else:
                    self.message_queue.put(f"Texto transcrito ({self.input_language}): {texto_original}")
